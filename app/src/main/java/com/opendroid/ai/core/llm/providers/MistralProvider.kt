...
class MistralProvider @Inject constructor(
    ...
    override val availableModels: List<String> = listOf("mistral-large-latest", "mistral-medium", "mistral-small", "open-mixtral-8x7b")
    ...
    override suspend fun complete(request: LLMRequest): LLMResponse {
        ...
        client.newCall(httpRequest).execute().use { response ->
            if (!response.isSuccessful) {
                throw IOException("Mistral request failed: Code ${response.code} - ${response.body?.string()}")
            }
    ...
    override fun streamComplete(request: LLMRequest): Flow<String> = flow {
        try {
            ...
            emit("Error streaming Mistral: ${e.localizedMessage}")
        }
    }
